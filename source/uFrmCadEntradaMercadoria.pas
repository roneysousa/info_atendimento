...
uses UdmDados, UFrmMain, uFuncoes, uFrmFecharEntrada, uFrmLocalizaProduto,
  udmFinanceiro, uFrmLocFornecedor;
...
begin
     ...
     Try
        ...
        //
        cmbLoja.SetFocus;}
        dbeNRPEDI.Text := uFuncoes.StrZero(FloattoStr(dmDados.cdsConfigCFG_SEQENTRADA.Value+1),10);
        lbl_NRPEDIDO.Caption := uFuncoes.StrZero(FloattoStr(dmDados.CdsConfigCFG_SEQENTRADA.Value+1),10);
        dbeDTRECE.Text  := DatetoStr(Date());
        //
        M_NRPEDI := StrtoInt(dbeNRPEDI.Text);
        //
        If not (dmFinanceiro.cdsProdutosEntrada.Active) Then
            dmFinanceiro.cdsProdutosEntrada.Open;
        //
        If (edtCDFORN.Enabled) Then
           edtCDFORN.SetFocus;
     Except
        on Exc:Exception do
        begin
...
begin
     If (M_TOENTR = 0) AND (dmFinanceiro.cdsProdutosEntrada.IsEmpty) Then
     Begin
     ...
     Try
        ...
        try
          ...
          M_VALIPI := 0;
          //
          With dmFinanceiro.cdsProdutosEntrada do
          Begin
               DisableControls;
               First;
               //
...
begin
     ...
     Begin
           If not (dmDados.ProcurarValor(edtCDFORN.Text, 'FOR_CODIGO', 'FORNECEDORES')) Then
           begin
           ...
           Begin
             If not uFuncoes.Empty(edtCDFORN.text)
               ...
               End;
               //
               lbl_NMFORN.Text := DMDados.GetNomeForn(StrtoInt(edtCDFORN.text));
...
begin
     ...
     //
     If not (dmFinanceiro.cdsProdutosEntrada.IsEmpty) Then
...
begin
      ...
      Finally
          If not (frmConsFornecedor.cdsConsultar.IsEmpty) Then
           begin
               edtCDFORN.Text := uFuncoes.StrZero(frmConsFornecedor.cdsConsultarFOR_CODIGO.AsString,5);
               dbeDTPEDI.SetFocus;
           End;
...
begin
    ...
    try
        ...
        begin
            ...
            //DMDados.cdsItensVendaCDS_CDPROD.AsInteger :=
            cmbUNIDADE.SetFocus;
...
begin
     ...
     Begin
          {If (DMDados.ValidarProdutoServico(edtCDPROD.Text)) Then
            begin
            ...
            End;}
            //
            texto := 'Select PRO_CODIGO, PRO_BARRAS, PRO_DESCRICAO, PRO_UNIDADE,  ';
            texto := texto + 'PRO_VLVENDA, PRO_VLCOMPRA, PRO_FLPROD from PRODUTOS Where (PRO_CODIGO = :pCODIGO) ';
            texto := texto + ' OR (PRO_BARRAS = :pBARRAS) ';
            ...
            with QrAux do
              try
                ...
                If not (IsEmpty) Then
                 begin
                      If (FieldByName('PRO_FLPROD').AsString = 'S') Then
                      Begin
                          ...
                          Exit;
                      End;
                      lbl_NMPROD.Text := FieldByName('PRO_DESCRICAO').AsString;
                      edtValor.Value     := FieldByName('PRO_VLCOMPRA').AsCurrency;
                      cmbUNIDADE.Items.Add(FieldByName('PRO_UNIDADE').AsString);
                      edtCDPROD.Text := FieldByName('PRO_CODIGO').AsString;
                      //
...
begin
     ...
     Begin
          texto := 'Select ENT_FORNECEDOR, ENT_NUMERONOTA from ENTRADAS Where (ENT_FORNECEDOR = :pCODIGO)';
          texto := texto + ' and (ENT_NUMERONOTA = :pNRNOTA) and (ENT_SERIENOTA = :pSERIENOTA)';
          //
          QrAux := TSqlquery.Create(nil);
          with QrAux do
            try
