object frmManuContasPagar: TfrmManuContasPagar
  Left = 191
  Top = 123
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Manuten'#231#227'o de Contas '
  ClientHeight = 511
  ClientWidth = 753
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 753
    Height = 113
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 17
      Top = 11
      Width = 38
      Height = 13
      Caption = '&Per'#237'odo'
      FocusControl = edtDTINICIAL
      Transparent = True
    end
    object Label3: TLabel
      Left = 271
      Top = 11
      Width = 57
      Height = 13
      Caption = '&Fornecedor:'
      FocusControl = cmbFornecedores
    end
    object edtDTINICIAL: TDateEdit
      Left = 17
      Top = 27
      Width = 121
      Height = 21
      NumGlyphs = 2
      TabOrder = 0
      OnExit = edtDTINICIALExit
    end
    object edtDTFINAL: TDateEdit
      Left = 144
      Top = 27
      Width = 121
      Height = 21
      NumGlyphs = 2
      TabOrder = 1
      OnExit = edtDTFINALExit
    end
    object cmbFornecedores: TComboBox
      Left = 271
      Top = 26
      Width = 286
      Height = 22
      Style = csOwnerDrawFixed
      ItemHeight = 16
      TabOrder = 2
    end
    object rgPeriodos: TRadioGroup
      Left = 271
      Top = 54
      Width = 286
      Height = 41
      Caption = '[ Per'#237'odos ]'
      Columns = 3
      ItemIndex = 0
      Items.Strings = (
        '&Compra'
        '&Vencimento'
        'Pa&gamento')
      TabOrder = 4
    end
    object rgSituacao: TRadioGroup
      Left = 16
      Top = 54
      Width = 249
      Height = 41
      Caption = '[ Situa'#231#227'o ]'
      Columns = 3
      ItemIndex = 0
      Items.Strings = (
        '&Abertas'
        '&Pagas'
        '&Todas')
      TabOrder = 3
    end
    object btFiltar: TBitBtn
      Left = 568
      Top = 64
      Width = 92
      Height = 25
      Caption = 'Fil&tra'
      TabOrder = 5
      OnClick = btFiltarClick
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFA98FAFF00FFFF00FF}
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 113
    Width = 753
    Height = 311
    Align = alClient
    TabOrder = 1
    object grdConsultar: TDBGrid
      Left = 1
      Top = 1
      Width = 751
      Height = 309
      Align = alClient
      Ctl3D = False
      DataSource = dsConsulta
      Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'PAG_IDCONTA'
          Title.Alignment = taCenter
          Title.Caption = 'Compra'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 55
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PAG_NRPARCELA'
          Title.Alignment = taCenter
          Title.Caption = 'PC'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 42
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PAG_DTCOMPRA'
          Title.Caption = 'Dt.Compra'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PAG_DTVENCIMENTO'
          Title.Alignment = taCenter
          Title.Caption = 'Dt.Vencimento'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 78
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PAG_DTPAGAMENTO'
          Title.Alignment = taCenter
          Title.Caption = 'Dt.Pagamento'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 75
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PAG_ATRASO'
          Title.Alignment = taCenter
          Title.Caption = 'Atraso'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 54
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PAG_VALORPARCELA'
          Title.Alignment = taRightJustify
          Title.Caption = 'Valor'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 97
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PAG_VALORPAGO'
          Title.Alignment = taRightJustify
          Title.Caption = 'Valor Pago'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 88
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PAG_NMFORN'
          Title.Caption = 'Fornecedor'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 200
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PAG_NMSITUACAO'
          Title.Caption = 'Situa'#231#227'o'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PAG_NMBANC'
          Title.Alignment = taCenter
          Title.Caption = 'Banco'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 200
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PAG_AGENCIA'
          Title.Alignment = taCenter
          Title.Caption = 'Ag'#234'ncia'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PAG_NRCONTA'
          Title.Alignment = taRightJustify
          Title.Caption = 'N'#186'.Conta'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PAG_NRCHEQUE'
          Title.Alignment = taRightJustify
          Title.Caption = 'N'#186'.Cheque'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end>
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 424
    Width = 753
    Height = 46
    Align = alBottom
    TabOrder = 2
    object Label4: TLabel
      Left = 19
      Top = 7
      Width = 148
      Height = 13
      Caption = 'TOTAL DE ABERTAS ==>'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Label5: TLabel
      Left = 19
      Top = 28
      Width = 147
      Height = 13
      Caption = 'TOTAL DE PAGAS   ===>'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object lbl_TOTABERTAS: TLabel
      Left = 264
      Top = 5
      Width = 29
      Height = 16
      Alignment = taRightJustify
      Caption = '0,00'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object lbl_PAGAS: TLabel
      Left = 264
      Top = 27
      Width = 29
      Height = 16
      Alignment = taRightJustify
      Caption = '0,00'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
  end
  object Panel4: TPanel
    Left = 0
    Top = 470
    Width = 753
    Height = 41
    Align = alBottom
    Color = clNavy
    TabOrder = 3
    object BtEditar: TBitBtn
      Left = 19
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Editar Parcela'
      Caption = '&Editar'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = BtEditarClick
      Glyph.Data = {
        
        3F3F3F3F39111616191919193334343434333334270D19191919}
    end
    object BtExcluir: TBitBtn
      Left = 104
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Excluir'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BtExcluirClick
      Glyph.Data = {
        
        0404040404040404040404040404040404040404040404040404}
    end
    object btBaixa: TBitBtn
      Left = 223
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Baixa'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = btBaixaClick
      Glyph.Data = {
        
        00333377737FFFFF773333303300000003333337337777777333}
      NumGlyphs = 2
    end
    object BtCancelar: TBitBtn
      Left = 394
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Cancelar'
      Caption = '&Cancelar'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      Visible = False
      Glyph.Data = {
        
        2828282828282828282828282828282828282828282828282828}
    end
    object BtGravar: TBitBtn
      Left = 477
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Gravar altera'#231#227'o'
      Caption = '&Gravar'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      Visible = False
      Glyph.Data = {
        
        4343434343434630130F4848092D3A363636363636363A2A0748}
    end
    object BtSair: TBitBtn
      Left = 652
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Fechar janela'
      Caption = '&Sair'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      OnClick = BtSairClick
      Glyph.Data = {
        
        1B1C010303030303031A1A1A1A1A1A1A0303011A1A1A1A1A1A1A}
    end
  end
  object dspConsulta: TDataSetProvider
    DataSet = dstConsulta
    Options = [poAllowCommandText]
    Left = 448
    Top = 193
  end
  object dsConsulta: TDataSource
    AutoEdit = False
    DataSet = cdsConsulta
    OnDataChange = dsConsultaDataChange
    Left = 536
    Top = 193
  end
  object dstConsulta: TSQLDataSet
    CommandText = 
      'Select PAG_SEQUENCIA, PAG_IDCONTA, PAG_NRPARCELA, PAG_CDLOJA,'#13#10'P' +
      'AG_DTCOMPRA, PAG_DTVENCIMENTO, PAG_DTPAGAMENTO, PAG_VALORPARCELA' +
      ', PAG_IDFORNECEDOR,'#13#10'PAG_SITUACAO, PAG_VALORPAGO, PAG_BANCO, PAG' +
      '_NRCONTA, PAG_AGENCIA,'#13#10'PAG_NRCHEQUE from contaspagar'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = dmDados.sqlConexao
    Left = 408
    Top = 193
  end
  object cdsConsulta: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspConsulta'
    OnCalcFields = cdsConsultaCalcFields
    Left = 488
    Top = 193
    object cdsConsultaPAG_SEQUENCIA: TIntegerField
      FieldName = 'PAG_SEQUENCIA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      DisplayFormat = '0000000'
    end
    object cdsConsultaPAG_IDCONTA: TIntegerField
      FieldName = 'PAG_IDCONTA'
      Required = True
      DisplayFormat = '0000000'
    end
    object cdsConsultaPAG_NRPARCELA: TIntegerField
      FieldName = 'PAG_NRPARCELA'
      Required = True
      DisplayFormat = '00'
    end
    object cdsConsultaPAG_CDLOJA: TIntegerField
      FieldName = 'PAG_CDLOJA'
    end
    object cdsConsultaPAG_DTCOMPRA: TDateField
      Alignment = taRightJustify
      FieldName = 'PAG_DTCOMPRA'
      EditMask = '99/99/9999;1;_'
    end
    object cdsConsultaPAG_DTVENCIMENTO: TDateField
      Alignment = taRightJustify
      FieldName = 'PAG_DTVENCIMENTO'
      EditMask = '99/99/9999;1;_'
    end
    object cdsConsultaPAG_DTPAGAMENTO: TDateField
      Alignment = taRightJustify
      FieldName = 'PAG_DTPAGAMENTO'
      EditMask = '99/99/9999;1;_'
    end
    object cdsConsultaPAG_VALORPARCELA: TFMTBCDField
      FieldName = 'PAG_VALORPARCELA'
      DisplayFormat = '###,##0.#0'
      EditFormat = '###,##0.#0'
      Precision = 18
      Size = 2
    end
    object cdsConsultaPAG_IDFORNECEDOR: TIntegerField
      FieldName = 'PAG_IDFORNECEDOR'
    end
    object cdsConsultaPAG_SITUACAO: TStringField
      FieldName = 'PAG_SITUACAO'
      FixedChar = True
      Size = 1
    end
    object cdsConsultaPAG_VALORPAGO: TFMTBCDField
      FieldName = 'PAG_VALORPAGO'
      DisplayFormat = '###,##0.#0'
      EditFormat = '###,##0.#0'
      Precision = 18
      Size = 2
    end
    object cdsConsultaPAG_BANCO: TIntegerField
      FieldName = 'PAG_BANCO'
      DisplayFormat = '000'
    end
    object cdsConsultaPAG_NRCONTA: TStringField
      FieldName = 'PAG_NRCONTA'
      Size = 12
    end
    object cdsConsultaPAG_AGENCIA: TStringField
      FieldName = 'PAG_AGENCIA'
      EditMask = '#####-A;0;_'
      Size = 6
    end
    object cdsConsultaPAG_NRCHEQUE: TStringField
      FieldName = 'PAG_NRCHEQUE'
      Size = 10
    end
    object cdsConsultaPAG_NMSITUACAO: TStringField
      FieldKind = fkCalculated
      FieldName = 'PAG_NMSITUACAO'
      Size = 15
      Calculated = True
    end
    object cdsConsultaPAG_NMFORN: TStringField
      FieldKind = fkLookup
      FieldName = 'PAG_NMFORN'
      LookupDataSet = dmFinanceiro.cdsListaFornecedores
      LookupKeyFields = 'FOR_CODIGO'
      LookupResultField = 'FOR_FANTASIA'
      KeyFields = 'PAG_IDFORNECEDOR'
      Size = 50
      Lookup = True
    end
    object cdsConsultaPAG_ATRASO: TFloatField
      FieldKind = fkCalculated
      FieldName = 'PAG_ATRASO'
      Calculated = True
    end
    object cdsConsultaPAG_NMBANC: TStringField
      FieldKind = fkLookup
      FieldName = 'PAG_NMBANC'
      LookupDataSet = dmFinanceiro.cdsListaBanco
      LookupKeyFields = 'BAN_CODIGO'
      LookupResultField = 'BAN_NOME'
      KeyFields = 'PAG_BANCO'
      Size = 40
      Lookup = True
    end
  end
end
